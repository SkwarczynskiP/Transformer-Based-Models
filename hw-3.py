...
def tokenizeT5(text):
    return tokenizerT5(text["sms"], padding="max_length", truncation=True, max_length=256)
...
# Model 2: t5-small
print("\n\nModel 2: google-t5/t5-small")
tokenizerT5 = AutoTokenizer.from_pretrained("google-t5/t5-small")
tokenizedDatasetT5 = dataset.map(tokenizeT5, batched=True)
trainDatasetT5 = tokenizedDatasetT5['train'].shuffle(seed=42).select(range(4456))  # 80% Split of the dataset
testDatasetT5 = tokenizedDatasetT5['train'].shuffle(seed=42).select(range(1114))  # 20% Split of the dataset
modelT5 = AutoModelForSequenceClassification.from_pretrained("google-t5/t5-small", num_labels=2)
T5Training_args = TrainingArguments(output_dir="test_trainer")
metric = evaluate.load("accuracy")
...
predictionsT5 = trainerT5.predict(testDatasetT5)
classPredictionsT5 = np.argmax(predictionsT5.predictions, axis=-1)
evalMetricsT5 = computeMetrics((predictionsT5.predictions, predictionsT5.label_ids))
print(evalMetricsT5)
